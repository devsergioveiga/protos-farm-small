...
describe('CostCenterWizardModal', () => {
  ...
  it('renders step 1 with 5 asset-type radio cards when isOpen=true', () => {
    ...
    expect(screen.getByText('Maquina')).toBeInTheDocument();
    expect(screen.getByText('Veiculo')).toBeInTheDocument();
    expect(screen.getByText('Implemento')).toBeInTheDocument();
    expect(screen.getByText('Benfeitoria')).toBeInTheDocument();
  ...
  it('clicking a radio card and Proximo navigates to step 2', async () => {
    ...
    expect(screen.getByText(/sugest[aã]o de c[oó]digo/i)).toBeInTheDocument();
    expect(screen.getByText('MAQ')).toBeInTheDocument();
  });
  ...
  it('navigating to step 3 shows pre-filled code input with selected type prefix', async () => {
    ...
    expect((codeInput as HTMLInputElement).value).toBe('VEI-');
  });
  ...
  it('failed POST shows error banner with role=alert', async () => {
    ...
    expect(errorBanner).toHaveTextContent(/n[aã]o foi poss[ií]vel/i);
  });
