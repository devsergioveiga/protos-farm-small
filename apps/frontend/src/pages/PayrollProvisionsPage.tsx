...
const PayrollProvisionsPage = () => {
  ...
  const totalProvision = provisions
    .filter((p) => !p.reversedAt)
    .reduce((sum, p) => sum + p.provisionAmount, 0);
  const totalCharges = provisions
    .filter((p) => !p.reversedAt)
    .reduce((sum, p) => sum + p.chargesAmount, 0);
  const totalAmount = provisions
    .filter((p) => !p.reversedAt)
  ...
  return (
    <main className="prov-page" id="main-content">
      ...
      <nav className="prov-page__breadcrumb" aria-label="Localização na aplicação">
        ...
        <span className="prov-page__breadcrumb-sep" aria-hidden="true">
          /
        </span>
        <span className="prov-page__breadcrumb-item prov-page__breadcrumb-item--current">
          Provisoes
        </span>
      </nav>
      ...
      <header className="prov-page__header">
        ...
        <div className="prov-page__header-controls">
          <div className="prov-page__month-group">
            <label htmlFor="prov-month-select" className="prov-page__month-label">
              Mes de referencia
            </label>
            <select
        ...
        {loading ? (
          ...
          <div className="prov-page__empty">
            ...
            <p className="prov-page__empty-body">
              Clique em Calcular Provisoes para apurar ferias e 13o de todos os colaboradores
              ativos.
            </p>
            <button
              type="button"
          ...
          <div className="prov-page__table-section">
            <table className="prov-page__table">
              <caption className="sr-only">
                Provisoes do mes {getMonthLabel(selectedMonth)} agrupadas por colaborador
              </caption>
              <thead>
                <tr>
                  ...
                  <th scope="col" className="prov-page__col-numeric">
                    Salario Base
                  </th>
                  <th scope="col" className="prov-page__col-numeric">
                    Provisao
              ...
              <tbody>
                  ...
                  items.map((p, idx) => (
                    ...
                    >
                      ...
                      <td className="prov-page__col-numeric">
                        {formatCurrency(p.provisionAmount)}
                      </td>
                ...
                <tr className="prov-page__summary-row">
                  <td colSpan={4} className="prov-page__summary-label">
                    Total
                  </td>
      ...
      >
        <div className="prov-page__report-header">
          ...
          <button type="button" className="prov-page__export-btn" onClick={handleExport}>
        ...
        {loading ? (
          ...
          <div className="prov-page__empty">
            ...
            <p className="prov-page__empty-body">
              Calcule as provisoes do mes para visualizar o relatorio de posicao por centro de
              custo.
            </p>
          </div>
        ) : (
          <div className="prov-page__table-section">
            <table className="prov-page__table">
              <caption className="sr-only">
                Relatorio de posicao de provisoes por centro de custo
              </caption>
              <thead>
                <tr>
                  <th scope="col">Centro de Custo</th>
              ...
              <tbody>
                {report.map((row: ProvisionReportRow, i) => (
                  <tr key={row.costCenterId ?? `no-cc-${i}`} className="prov-page__row">
                    ...
                    <td className="prov-page__col-numeric">
                      {formatCurrency(row.thirteenthTotal)}
                    </td>
                    <td className="prov-page__col-numeric">{formatCurrency(row.chargesTotal)}</td>
                    <td className="prov-page__col-numeric prov-page__col-bold">
