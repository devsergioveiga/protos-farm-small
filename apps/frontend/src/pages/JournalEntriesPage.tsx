          ...
          {entry.status === 'POSTED' && (
            <li role="none">
              <button
                type="button"
                role="menuitem"
                className="je-page__menu-item je-page__menu-item--danger"
                onClick={() => {
                  setOpen(false);
                  onReverse();
                }}
    ...
    >
      <div className="je-page__csv-modal">
        ...
        <p className="je-page__csv-summary">
          {preview.totalEntries} lançamento{preview.totalEntries !== 1 ? 's' : ''} encontrado
          {preview.totalEntries !== 1 ? 's' : ''}
          {preview.totalErrors > 0 && (
            <span className="je-page__csv-errors">
              {' '}
              — {preview.totalErrors} erro{preview.totalErrors !== 1 ? 's' : ''} encontrado
        ...
        {preview.totalEntries > 0 && (
          <p className="je-page__csv-notice">
            Os {preview.totalEntries} lançamento{preview.totalEntries !== 1 ? 's' : ''} serão
            criados como rascunho. Revise antes de lançar.
          </p>
        )}
        ...
        <div className="je-page__csv-actions">
          ...
          {preview.totalEntries > 0 && (
            ...
            >
              {isLoading
                ? 'Importando...'
                : `Criar ${preview.totalEntries} rascunho${preview.totalEntries !== 1 ? 's' : ''}`}
            </button>
          )}
        </div>
      </div>
...
export default function JournalEntriesPage() {
  ...
  const handleCsvImportConfirm = async () => {
    ...
    try {
      ...
      showToast(
        `${successCount} rascunho${successCount !== 1 ? 's' : ''} criado${successCount !== 1 ? 's' : ''} com sucesso`,
      );
      setCsvPreview(null);
      setCsvFile(null);
      void refetch();
  ...
  return (
    <main className="je-page" id="main-content">
      ...
      <nav className="je-page__breadcrumb" aria-label="Caminho da página">
        ...
        <span className="je-page__bc-sep" aria-hidden="true">
          /
        </span>
        ...
        >
          ...
          {pendingCounts.error > 0 && (
            <span
              ...
              aria-label={`${pendingCounts.error} erros`}
            >
              {pendingCounts.error}
            </span>
          )}
          {pendingCounts.pending > 0 && (
            <span
              className="tab-badge tab-badge--warning"
              aria-label={`${pendingCounts.pending} pendentes`}
            >
      ...
      >
        ...
        <div className="je-page__filters">
          <div className="je-page__filter-group">
            <label htmlFor="je-filter-period" className="je-page__filter-label">
              Período
            </label>
            <select
              id="je-filter-period"
          ...
          <div className="je-page__filter-group">
            <label htmlFor="je-filter-status" className="je-page__filter-label">
              Status
            </label>
            <select
              id="je-filter-status"
          ...
          <div className="je-page__filter-group">
            <label htmlFor="je-filter-type" className="je-page__filter-label">
              Tipo
            </label>
            <select
              id="je-filter-type"
        ...
        <div className="je-page__table-wrapper">
          <table className="je-page__table">
            ...
            <thead>
              <tr>
                <th scope="col" className="je-page__th">
                  NÚMERO
                </th>
                <th scope="col" className="je-page__th">
                  DATA
                </th>
                <th scope="col" className="je-page__th je-page__th--wide">
                  HISTÓRICO
                </th>
                <th scope="col" className="je-page__th je-page__th--hidden-mobile">
                  TIPO
                </th>
                <th
                  scope="col"
                  className="je-page__th je-page__th--right je-page__th--hidden-mobile"
                >
                  VALOR TOTAL
                </th>
                <th scope="col" className="je-page__th">
                ...
                <th scope="col" className="je-page__th je-page__th--actions">
                  AÇÕES
                </th>
              </tr>
            </thead>
                  ...
                  <td colSpan={7}>
                    <div className="je-page__empty">
                      ...
                      <p className="je-page__empty-desc">
                        Crie o primeiro lançamento manual ou configure o saldo de abertura para
              ...
              {!isLoading &&
                entries.map((entry) => (
                  <tr key={entry.id} className="je-page__row">
                    <td className="je-page__td je-page__td--mono">#{entry.entryNumber}</td>
                    <td className="je-page__td je-page__td--mono">{formatDate(entry.entryDate)}</td>
                    ...
                    <td className="je-page__td je-page__td--hidden-mobile">
                      {entry.entryType === 'AUTOMATIC' ? (
                        <span
                          className="je-page__badge je-page__badge--automatic"
                          aria-label="Tipo: Automático"
                        >
            ...
            {entries.map((entry) => (
              <li key={entry.id} className="je-page__mobile-card">
                ...
                <div className="je-page__mobile-actions">
                  <button
                    type="button"
                    className="je-page__btn je-page__btn--sm je-page__btn--secondary"
                  ...
                  {entry.status === 'POSTED' && (
                    <button
                      type="button"
                      className="je-page__btn je-page__btn--sm je-page__btn--danger"
