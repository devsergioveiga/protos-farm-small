...
import { AlertTriangle, Wrench, ArrowRight, AlertCircle } from 'lucide-react';
...
export default function MaintenanceDashboardPage() {
  ...
  const patchWorkOrderStatus = updateWorkOrder as unknown as (
    id: string,
  ...
  const availabilityStr =
    dashboard?.availability !== null && dashboard?.availability !== undefined
      ? `${dashboard.availability.toFixed(1)}%`
      : null;

  const mtbfStr =
    dashboard?.mtbfHours !== null && dashboard?.mtbfHours !== undefined
      ? `${dashboard.mtbfHours.toFixed(1)} horas`
      : null;

  const mttrStr =
    dashboard?.mttrHours !== null && dashboard?.mttrHours !== undefined
      ? `${dashboard.mttrHours.toFixed(1)} horas`
      : null;

  const ytdStr = dashboard !== null ? formatBRL(dashboard?.totalCostYTD) : null;
  ...
  const hasNoData =
    !loading && dashboard !== null && dashboard.totalCostYTD === 0 && workOrders.length === 0;
  ...
  return (
    <main className="maintenance-dashboard">
      ...
      <nav className="maintenance-dashboard__breadcrumb" aria-label="Navegacao estrutural">
        ...
        <span className="maintenance-dashboard__breadcrumb-sep" aria-hidden="true">
          /
        </span>
        <span className="maintenance-dashboard__breadcrumb-item--current">
      ...
      {hasNoData && (
        <div className="maintenance-dashboard__empty">
          ...
          <p className="maintenance-dashboard__empty-desc">
            As metricas de disponibilidade e custo aparecerao aqui assim que a primeira OS for
            encerrada.
          </p>
        </div>
      )}
      ...
      {!hasNoData && (
        <div className="maintenance-dashboard__kpi-grid" aria-label="Indicadores de manutencao">
          <KpiCard label="Disponibilidade Mecanica" value={availabilityStr} loading={loading} />
          <KpiCard label="MTBF" value={mtbfStr} isMono tooltip={ND_TOOLTIP} loading={loading} />
          <KpiCard label="MTTR" value={mttrStr} isMono tooltip={ND_TOOLTIP} loading={loading} />
          <KpiCard label="Custo YTD" value={ytdStr} isMono loading={loading} />
        </div>
      )}
