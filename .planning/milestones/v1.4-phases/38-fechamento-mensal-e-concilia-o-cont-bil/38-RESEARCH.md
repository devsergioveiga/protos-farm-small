...
export function checkPeriodOpen() {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (!dateStr) {
      next();
      return;
    } // sem data = sem verificação (draft sem período)
...
export async function startClosing(
  ...
  if (period.status !== 'OPEN')
    throw new MonthlyClosingError('Período não está aberto', 'PERIOD_NOT_OPEN', 422);
  ...
  if (stepNumber > 1) {
    ...
    if (prevResults[prevKey]?.status !== 'OK') {
      throw new MonthlyClosingError(
        `Etapa ${stepNumber - 1} deve ser validada primeiro`,
        'STEP_DEPENDENCY',
        422,
      );
    }
