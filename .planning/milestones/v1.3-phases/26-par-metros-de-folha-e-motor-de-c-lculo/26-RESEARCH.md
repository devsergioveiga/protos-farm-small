...
export function calculateINSS(
  ...
  return {
    ...
    cappedAt: grossSalary.greaterThan(INSS_CEILING_2026) ? INSS_CEILING_2026 : null,
  };
}
```
...
export function calculateINSS(
  ...
  for (const bracket of brackets) {
    if (remaining.lessThanOrEqualTo(0)) break;
    const bracketWidth = bracket.upTo ? bracket.upTo.minus(bracket.from).plus('0.01') : remaining; // última faixa absorve o restante até o teto
...
export function calculateIRRF(input: IRRFInput): IRRFResult {
  const { grossSalary, inssContribution, dependents, alimony, brackets, dependentDeduction } =
    input;
...
export function calculateRuralUtility(
  ...
  return {
    housing: Decimal.min(requestedHousing, regionalMinWage.mul(HOUSING_MAX_RATE)).toDecimalPlaces(
      2,
      Decimal.ROUND_HALF_UP,
    ),
    food: Decimal.min(requestedFood, regionalMinWage.mul(FOOD_MAX_RATE)).toDecimalPlaces(
      2,
      Decimal.ROUND_HALF_UP,
    ),
  };
