...
export async function createEpiDelivery(
  ...
  return withRlsContext(ctx, async (tx) => {
    ...
    const delivery = await tx.epiDelivery.create({
      data: { ...input, stockOutputId: stockOutput.id },
    });
...
export function useEpiProducts() {
  ...
  const fetchEpiProducts = useCallback(async (params?: ListEpiProductsParams) => {
    ...
    try {
      ...
      // ... build params
      const data = await api.get<{ data: EpiProduct[]; total: number }>(
        `/epi-products?${searchParams}`,
      );
...
_(All spec files follow the exact pattern of `stock-alerts.routes.spec.ts` — supertest + jest.mock(service) + jest.mock(auth) + authAs helper)_
