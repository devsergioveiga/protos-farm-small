...
export async function getOperationalCost(
  ...
  const woWhere: Record<string, unknown> = {
    assetId, organizationId: ctx.organizationId, status: 'CONCLUIDA',
  };
  if (dateFilter) woWhere['completedAt'] = dateFilter;
  const woAgg = await prisma.workOrder.aggregate({
    where: woWhere as never, _sum: { totalCost: true },
  });
  ...
  const fuelAgg = await prisma.fuelRecord.aggregate({
    where: fuelWhere as never, _sum: { totalCost: true }, _count: { id: true },
  });
...
*(All OPER-01, OPER-02, OPER-03 test files exist. Only OPER-04 spec is missing.)*
