...
function projectAsset(asset: AssetWithConfig, horizonMonths: number): MonthlyProjection[] {
  let currentBookValue = new Decimal(
    String(asset.latestClosingBookValue ?? asset.acquisitionValue ?? 0),
  );
  ...
  for (let i = 0; i < horizonMonths; i++) {
    ...
    result.push({
      year,
      month,
      amount: output.depreciationAmount,
      closingBookValue: output.closingBookValue,
    });
...
function repairAlert(maintenanceCost: Decimal, acquisitionValue: Decimal | null) {
  ...
  if (ratio.gte(0.7)) return 'REPLACE';
  if (ratio.gte(0.6)) return 'MONITOR';
  return 'OK';
}
```
...
_(Existing test infrastructure covers the rest — no framework installation needed)_
